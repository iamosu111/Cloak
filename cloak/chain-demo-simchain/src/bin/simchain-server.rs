...
async fn web_aggregate(result_slice: web::Json<ResultSlice>) -> actix_web::Result<impl Responder>{
    info!("into web_aggregate");
    let result = aggregate(&result_slice);
    Ok(HttpResponse::Ok().json(result))
}

async fn web_decompose(param: web::Json<DecomposeParam>) -> actix_web::Result<impl Responder>{
    info!("into web_decompose");
    let result = Decompose(&param);
    Ok(HttpResponse::Ok().json(result))
}
...
async fn main() -> actix_web::Result<()> {
    ...
    HttpServer::new(|| {
        App::new()
            ...
            .route("/aggregate", web::post().to(web_aggregate))
            .route("/decompose", web::post().to(web_decompose))
    })
    .bind(opts.binding)?
    .run()
    .await?;
